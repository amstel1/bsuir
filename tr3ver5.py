import sys
from PyQt5.QtWidgets import (QMainWindow, QAction, qApp, QApplication, QPushButton, QDesktopWidget,QVBoxLayout,
...
class Example(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn5.clicked.connect(lambda: plotCan.plot_determined(self.comboBox,
...
class PlotCanvas(FigureCanvas):
    ...
    def plot_determined(self, zakon_sv, sposob_sv, mu_sv, sigma_sv,  trend_type, n_, a_, b_, c_):
        sns.set_style('darkgrid')
        plt.style.use('ggplot')
        ...
        if trend_type.currentText() == 'Линейный':
            if a_.text().isnumeric() and b_.text().isnumeric():
                a = float(a_.text())
                b = float(b_.text())
            ...
            if a_.text().isnumeric() and b_.text().isnumeric() and c_.text().isnumeric():
                a = float(a_.text())
                b = float(b_.text())
                c = float(c_.text()) 
                if sposob_sv.currentText() == 'None':
