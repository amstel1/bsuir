...
class Example(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn5.clicked.connect(lambda: self.pc.plot_determined(self.slider, self.comboBox, 
        ...
        self.btn2.clicked.connect(lambda: self.pc.plot_fitted(self.estimator, self.combo, self.data))     #plot fitted
    ...
    def find_all(self):
        ...
        for text in AllItems:
           ...
           print(text)
           self.fit_regression()
           self.pc.plot_fitted(self.estimator, self.combo, self.data)
           #plotCan.plot_fitted(self.estimator, self.combo, self.data))
           #lambda: plotCan.plot_fitted(self.estimator, self.combo, self.data)
    ...
    def find_best(self):
        ...
        for text in AllItems:
           ...
           print(text)
           self.fit_regression()
           metrics[text] = self.rmse
        max_item = min(metrics, key=metrics.get)
        self.combo.setCurrentText(max_item)
        self.fit_regression()
        self.pc.plot_fitted(self.estimator, self.combo, self.data)
        #self.combo.activated(lambda: plotCan.plot_fitted(self.estimator, self.combo, self.data))
    ...
    def openFile(self):
        try:
            ...
            elif self.filepath.endswith('bin'):
                with open(self.filepath,'rb') as f:
                    bts = f.read()
                arr = (x for x in bts)
                self.data = pd.DataFrame(data = arr, columns = ['y'])
                self.data['t'] = [x for x in range(1,len(self.data)+1)]
    ...
    def fit_regression(self):
        try:
            ...
            if self.combo.currentText() =='Линейный':
                ...
                #y = alpha*cos(w*t)+beta*sin(w*t)
                t = 2
                
                self.data.loc[:, "cos_wt"] = np.cos(self.data.loc[:, "t"]  * t)
                self.data.loc[:, "sin_wt"] = np.sin(self.data.loc[:, "t"]  * t)
                X1 = self.data.loc[:, "cos_wt"].values.reshape(-1,1)
                X2 = self.data.loc[:, "sin_wt"].values.reshape(-1,1)
                self.estimator = LinearRegression(fit_intercept = True)
                fitted_estimator = self.estimator.fit(X = np.hstack((X1 , X2 )), y = y)
                #fitted_estimator = self.estimator.fit(X = np.hstack((X1+self.data.loc[:, "t"].values.reshape(-1,1) , X2+self.data.loc[:, "t"].values.reshape(-1,1) )), y = y)
                inter = fitted_estimator.intercept_[0]
                y_pred = fitted_estimator.coef_[0][0] * np.cos(X1*t) + fitted_estimator.coef_[-1][-1] * np.sin(X2*t)+inter
                #self.equation = "Заглушка0"
                self.equation = f"y = {round(fitted_estimator.coef_[0][0],2)} * cos({t}*x) + {round(fitted_estimator.coef_[-1][-1],2)} * sin({t}*x);"
            self.equation +=  '\n'
            self.equation +=  f'RMSE = {round(mean_squared_error(y_true = y, y_pred = y_pred)**0.5,2)}'
            self.rmse =  mean_squared_error(y_true = y, y_pred = y_pred)**0.5
    ...
    def plot_fitted(self,estimator = None, combo = None, data = np.empty(shape=(1,2))):
        ...
        try:
            ...
            if not 'Наблюдения' in self.axes.legend_.texts[0]._text:
                if combo.currentText() !='Гармонический': 
                    self.ax.plot(data['t'], data['y'], marker='.', color='black', linewidth=3, alpha=0.5, label="Наблюдения")  # 
                else:
                    self.ax.plot(data['t'], data['y'], marker='.', color='black', linewidth=1, alpha=0.1, label="Наблюдения")  # 
                self.ax.legend()
                self.draw()
            if isinstance(estimator, LinearRegression) and combo.currentText() =='Линейный':
                ...
                t = 2
                data.loc[:, "cos_wt"] = np.cos(data.loc[:, "t"]  * t)
                data.loc[:, "sin_wt"] = np.sin(data.loc[:, "t"]  * t)
                X1 = data.loc[:, "cos_wt"].values.reshape(-1,1)
                X2 = data.loc[:, "sin_wt"].values.reshape(-1,1)
                trend_eq = "?"
                inter = estimator.intercept_[0]
                data['y_hat'] = estimator.coef_[0][0] * np.cos(X1*t) + estimator.coef_[-1][-1] * np.sin(X2*t) + inter
                #data['y_hat'] = estimator.coef_[0][0] * np.cos(X1*t+data.loc[:, "t"].values.reshape(-1,1)) + estimator.coef_[-1][-1] * np.sin(X2*t+data.loc[:, "t"].values.reshape(-1,1)) + inter
                self.ax.plot(data['t'], data['y_hat'], marker='+', linewidth=0.5, alpha=0.5, label="Гарм. тренд: " + trend_eq)
            self.ax.set_title("Title", fontsize=12, fontweight=0)
            self.ax.set_xlabel("xlabel")
            self.ax.set_ylabel("ylabel")
            self.ax.legend()
