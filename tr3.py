    ...
    def fit_regression(self):
        try:
            if self.combo.currentText() =='Линейная':
                ...
                self.equation = f'y = {round(fitted_estimator.intercept_[0],2)} + {round(fitted_estimator.coef_[0][0],2)} * x; R-squared = {round(fitted_estimator.score(X = self.data.loc[:, 0].values.reshape(-1,1), y = self.data.loc[:, 1].values.reshape(-1,1)),2)}'
                ...
                self.equation = (f'y = {round(fitted_estimator.intercept_[0],2)} + {round(fitted_estimator.coef_[0][0],2)} * ln(x); R-squared = {round(fitted_estimator.score(X = self.data.loc[:, 0].values.reshape(-1,1), y = self.data.loc[:, 1].values.reshape(-1,1)),2)}')
                self.eq_label.setText(self.equation)
